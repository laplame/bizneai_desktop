        ...
        try {
            const status = {
                blockchain: {
                    ...
                    activeValidators: blockchain.participants.getActiveNodes().length,
                    consensus: blockchain.consensus,
                    lastBlock: blockchain.getLatestBlock(),
                    syncing: blockchain.isSyncing || false
                },
                network: {
                    connections: p2pManager.peers.size,
                    peers: Array.from(p2pManager.peers.entries()).map(([id, peer]) => ({
                        id,
                        latency: peer.latency || 0,
                        connected: true,
                        address: peer.remotePeer?.toString()
                    })),
                    localAddress: p2pManager.node?.peerId?.toString(),
