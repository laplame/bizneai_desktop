...
**Menú (BizneAI Chat / restaurante):** el catálogo público que en web se muestra como `/restaurant/:shopId/menu` proviene del mismo conjunto de productos del negocio. En la app, el menú para el asistente se obtiene vía este endpoint (JSON) o incluido en `GET /mcp/:shopId` con `includeProducts=true`. Servicio dedicado: `bizneaiMenuService.fetchShopMenuFromApi()`. Las sugerencias de nuevos ítems en chat contrastan estos datos con la BD local (`menuLocalContrast` + prompt en `aiService`).

### GET /products (catálogo general)

Listado con filtros: `category` (regex, case insensitive), `mainCategory`, `status` (por defecto `active`), `page`, `limit` (máximo 200), `search`, `minPrice`, `maxPrice`, `sortBy`, `sortOrder`. En este listado **no** se acota por `businessId`; para tienda usar la ruta siguiente.

**Servicio:** `generalCatalogApiService.fetchGeneralProductsList()`

### GET /products/by-category/:category

Misma idea de categoría por regex en el path. Query opcional: `businessId`, `mainCategory`, `page`, `limit`, etc. Útil para comprobar duplicados solo en una tienda.

**Servicio:** `generalCatalogApiService.fetchProductsByCategoryAndBusiness()`

**Duplicados (foto / Gemini en BizneAI Chat):** la API devuelve un documento por producto; el cliente cruza por nombre normalizado y sku/barcode. `duplicateCatalogService.fetchServerCatalogRowsForDuplicateCheck()` combina `GET /mcp/:shopId/products?search=` (tienda) +, si Gemini devolvió categoría, `GET /products/by-category/...?businessId=`. Clasificación: `menuLocalContrast.classifyCatalogDuplicate()`.
...
| Campo | Tipo | Descripción |
|-------|------|-------------|
| id | string | ID del cliente (ej. plaza_a1b2c3d4) |
| name | string | Nombre |
| rfc | string | RFC fiscal |
| status | string | active, inactive |
| allowCredit | boolean | Permitir crédito |
| notes | string | Notas |
| taxData | object | Datos fiscales CFDI (rfc, legalName, cfdiUse, taxRegime, postalCode, fiscalAddress, email, phone) |
| priceType | string | Opcional; tier / lista de precios |
| priceTypeCustomLabel | string | Opcional; etiqueta si tipo custom |
| commercialConditions | object \| null | Opcional; ver doc de clientes; `null` en PUT borra condiciones |

**Servicio:** `crudEventListenerService`, `customerDatabase`
...
Mismo body que POST (campos opcionales). Parámetro en path: `customerId`.
...
Query opcional: `status=active` \| `status=inactive` (sin query = todos).

**Servicio:** `customerSyncService.syncCustomersFromMcpServer()` (pantalla Clientes)

---

## 4. MCP - Caja Registradora (Cash Register)

### POST /mcp/:shopId/cash-register/open
**Abrir caja**
...
| clientSessionId | string | ID sesión del cliente (CS-timestamp) |

**Servicio:** `crudEventListenerService`, `cashierDatabase`

---

### POST /mcp/:shopId/cash-register/close
**Cerrar caja**
...
### POST /mcp/:shopId/cash-register/movements
**Registrar movimiento**

| Campo | Tipo | Descripción |
|-------|------|-------------|
| type | string | saleCash, refundCash, cashIn, cashOut, adjustment |
...
### GET /mcp/:shopId/cash-register/status
**Estado y balance actual**

Sin body. Retorna hasActiveSession, sessionId, balance, etc.

**Servicio:** (para sincronización o dashboard)

---

### GET /mcp/:shopId/cash-register/sessions
...
| customer | create | `/mcp/:shopId/customers` | POST |
| customer | update | `/mcp/:shopId/customers/:customerId` | PUT |
| customer | delete | `/mcp/:shopId/customers/:customerId` | DELETE |
| cashier_session | create | `/mcp/:shopId/cash-register/open` | POST |
| cashier_session | update | `/mcp/:shopId/cash-register/close` | POST |
| cashier_movement | create | `/mcp/:shopId/cash-register/movements` | POST |
...
| crudEventListenerService | POST/PUT/DELETE /mcp/:shopId/products, POST /mcp/:shopId/sales, PUT/DELETE /:shopId/sales/:id, POST /mcp/:shopId/inventory/adjust, POST /mcp/:shopId/cash-register/open, POST /mcp/:shopId/cash-register/close, POST /mcp/:shopId/cash-register/movements |
